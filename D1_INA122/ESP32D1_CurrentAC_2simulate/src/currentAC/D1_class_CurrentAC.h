//_____D1_class_CurrentAC.h___________________201203-201203_____
...
const int   x4maxD1mini_     = 947;    // ain=947 -> iAC=2A
...
const int   x4maxESP32D1_     = 3920;  // ain=3920 -> iAC=2A
...
class CurrentAC {
 ...
 protected:
  ...
  int    x4max_;                       // x-value for max (2A)
  int    x4maxDefault_;                // default x for max (2A)
 ...
 protected:
  void setup(int boardType, float ion_A, int pin);
 ...
 //------set parameter------------------------------------------
  void   setCurrentOn(float onAmpere); // current limit for "on"
  bool   setx4max(int x4max);          //ADC value for max current
  void   setNumberOf50HzPeriods(int periods); //number 50Hz per.
